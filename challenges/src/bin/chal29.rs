...
fn main() {
    ...
    let padding = chal29::get_md_padding(&random_bytes(key_size + original_msg.len() as u32));
...
fn deduce_key_size(mac: &SecretPrefixMac) -> Option<u32> {
    ...
    for key_size in 0..256 {
        let padding = chal29::get_md_padding(&random_bytes(key_size));
