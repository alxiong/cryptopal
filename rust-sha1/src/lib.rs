...
/// Remaining data block to be input into compression function
/// NOTE: only made public due to demo of extension attack
pub struct Blocks {
    len: u32,
    block: [u8; 64],
}

/// Represents a Sha1 registers (a, b, c, d, e) states
...
pub struct Digest {
    /// data of the Digest is the Sha1 register state
    pub data: Sha1State,
}
...
impl Sha1 {
    ...
    /// Set all internal states of Sha1
    pub fn set(state: Sha1State, blocks: Blocks, len: u64) -> Sha1 {
        Sha1 { state, blocks, len }
    }
    ...
    pub fn digest(&self) -> Digest {
        ...
        // NOTE by alex:
        // extra/MD padding is a 64-bit big-endian integer, thus the following declaration
        let extra = [
            (bits >> 56) as u8,
            (bits >> 48) as u8,
            (bits >> 40) as u8,
    ...
    /// Get digest result from a user-padded input
    /// @author: Alex Xiong
    pub fn digest_from_padded_input(&mut self, data: &[u8]) -> Digest {
        self.update(data);
